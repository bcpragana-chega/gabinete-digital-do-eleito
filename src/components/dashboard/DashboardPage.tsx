...
function hrefComParametros(to: string, params?: Record<string, string>) {
  return Object.entries(params ?? {}).reduce(
    (href, [param, value]) => href.replace(`$${param}`, encodeURIComponent(value)),
    to,
  );
}
      ...
      <div className="relative z-10 flex h-full flex-col">
        ...
        <div className="mt-auto pt-6">
          ...
          <div className="mt-6 flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
            ...
            >
              <a href={hrefComParametros(mission.to, mission.params)}>
                {mission.button}
...
function TaskRow({ task }: { task: TaskItem }) {
  ...
  return (
    <a
      href={hrefComParametros(task.to, task.params)}
...
function AlertRow({ alert }: { alert: AlertItem }) {
  ...
  return (
    <a
      href={hrefComParametros(alert.to, alert.params)}
      className={cn(
      ...
      <div className="space-y-2">
        {actions.map((action) => {
          ...
          return (
            <a
              key={action.label}
              href={hrefComParametros(action.to, action.params)}
