...
function DossieCard({ dossie }: { dossie: Dossie }) {
  ...
  return (
    <Link to="/assuntos/$dossieId" params={{ dossieId: dossie.id }} className="group block min-w-0">
      <Card className="flex min-h-52 min-w-0 flex-col overflow-hidden p-4 transition-colors duration-150 hover:bg-muted/15">
        <div className="flex shrink-0 items-start justify-between gap-4">
          <div className={ds.icon.tile}>
            <Folder className={ds.icon.sm} strokeWidth={1.75} />
          </div>
        </div>
        ...
        <div className="mt-3 min-w-0 overflow-hidden">
          ...
          <p className="mt-1.5 line-clamp-2 break-words text-xs leading-5 text-muted-foreground">
        ...
        <div className="mt-3 border-t border-border/60 pt-3">
          ...
          <p className="mt-1 text-xs font-medium text-foreground">{proximaAcaoDoAssunto(dossie)}</p>
        </div>

        <div className="mt-auto flex shrink-0 items-end justify-between gap-4 pt-3 text-xs">
