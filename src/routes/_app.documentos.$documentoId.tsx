...
function DocumentoDoAssuntoPage() {
            ...
            <EmptyState
              ...
              action={
                <div className="flex flex-wrap justify-center gap-2">
                  <Button asChild>
                    <Link to="/assuntos">Voltar aos assuntos</Link>
                  </Button>
                  {podeTentarNovamente && (
                    <Button
                      type="button"
  ...
  const hrefRegresso = documento.assuntoId
    ? `/assuntos/${encodeURIComponent(documento.assuntoId)}`
    : documento.assembleiaId
      ? `/sessoes/${encodeURIComponent(documento.assembleiaId)}`
      : "/assuntos";
      ...
      <main className="min-h-screen bg-transparent">
        <div className="mx-auto max-w-7xl px-4 py-6 sm:px-6 lg:px-8 lg:py-10">
          <div className="mb-6 flex flex-wrap items-center justify-between gap-4">
            <Breadcrumb
              items={[
                { label: documento.assuntoId ? "Assuntos" : "Documentos" },
                ...(dossie ? [{ label: dossie.titulo }] : []),
                { label: "Documento" },
              ]}
            />
