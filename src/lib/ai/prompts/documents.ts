...
export function construirPromptDocumento(contexto: ContextoGeracaoDocumento) {
  ...
  const institutional = contexto.institutionalContext;
  const contextoInstitucionalBloco = [
    "CONTEXTO INSTITUCIONAL VALIDADO",
    `- Nome do eleito: ${institutional.electedOfficial.name}`,
    `- Cargo institucional: ${institutional.electedOfficial.institutionalTitle}`,
    `- Papel institucional normalizado: ${institutional.electedOfficial.role}`,
    `- Nível territorial: ${institutional.territory.level}`,
    `- Freguesia: ${institutional.territory.parishName ?? ""}`,
    `- Município: ${institutional.territory.municipalityName}`,
    `- Órgão deliberativo: ${institutional.institution.deliberativeBody.officialName}`,
    `- Órgão executivo: ${institutional.institution.executiveBody.officialName}`,
    `- Sessão: ${institutional.session?.officialBodyName ?? ""}`,
    `- Destinatário: ${institutional.legal.recipient.officialName}`,
    `- Grupo de competências: ${institutional.legal.competenceGroup}`,
    `- Validação institucional: ${institutional.validation.valid ? "válida" : "inválida"}`,
    "- Base jurídica selecionada:",
    ...institutional.legal.articles.map(
      (artigo) => `  - ${artigo.diploma}, artigo ${artigo.article}: ${artigo.purpose}`,
    ),
  ].join("\n");
  const baseJuridicaBloco = [
    ...
    `- Nível de confiança jurídica: ${baseJuridica.nivelConfianca}`,
    `- Validação jurídica determinística: ${baseJuridica.valido ? "válida" : "inválida"}`,
    baseJuridica.motivoInvalido ? `- Motivo de bloqueio: ${baseJuridica.motivoInvalido}` : "",
    "- Diplomas confirmados pelo Tribuno:",
    ...baseJuridica.diplomas.map(
      (diploma) =>
        `  - ${diploma.numero}: ${diploma.titulo}. Fonte oficial: ${diploma.fonteOficial}. ${diploma.versaoConsultada}`,
    ),
    "- Artigos concretamente aplicáveis:",
    ...baseJuridica.artigosAplicaveis.map((artigo) => {
      const detalhe = [artigo.numero ? `n.º ${artigo.numero}` : "", artigo.alinea ?? ""]
        .filter(Boolean)
        .join(", ");
      return `  - ${artigo.diploma}, artigo ${artigo.artigo}${detalhe ? `, ${detalhe}` : ""}: ${artigo.finalidade}`;
    }),
    "- Enquadramento jurídico relevante:",
    ...baseJuridica.enquadramentoJuridico.map((item) => `  - ${item}`),
    "- Competências deliberativas do órgão de apresentação:",
    ...baseJuridica.competenciasPorPapel.deliberativas.map((item) => `  - ${item}`),
    "- Competências de fiscalização do órgão de apresentação:",
    ...baseJuridica.competenciasPorPapel.fiscalizacao.map((item) => `  - ${item}`),
    "- Competências executivas do órgão de apresentação:",
    ...baseJuridica.competenciasPorPapel.executivas.map((item) => `  - ${item}`),
    "- Competências consultivas/recomendatórias do órgão de apresentação:",
    ...baseJuridica.competenciasPorPapel.consultivasRecomendatorias.map((item) => `  - ${item}`),
  ...
  return [
    ...
    "- Utiliza exatamente os nomes institucionais fornecidos no CONTEXTO INSTITUCIONAL VALIDADO.",
    "- Não alteres, completes ou recomponhas nomes de órgãos.",
    "- Não cries destinatários alternativos.",
    ...
    "- Identifica a competência do órgão a que o documento se destina apenas a partir do CONTEXTO INSTITUCIONAL VALIDADO e da Base Jurídica Institucional.",
    ...
    `- Município estruturado: ${contexto.perfil.municipio ?? ""}`,
    `- Freguesia estruturada: ${contexto.perfil.freguesia ?? ""}`,
